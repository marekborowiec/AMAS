...
class Alignment:
    """Base class: Gets in parsed sequences as input and summarizes their stat.
    Subclasses (AminoAcidAlignment, DNAAlignment) should define the following based on data type:
    """
    # linter
    alphabet = []
    missing_ambiguous_chars = []
    missing_chars = []
    non_alphabet = []
...
class MetaAlignment():
    ...
    def write_partitions(self, file_name, part_format, codons):
        ...
        self.file_overwrite_error(file_name)
        with open(file_name, "w", encoding="utf-8") as concat_part:
            if part_format == "nexus":
                concat_part.write(self.print_nexus_partitions(codons))
            elif part_format == "raxml":
